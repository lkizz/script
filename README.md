...
2. 如果你只想无脑使用上游，不想自己修改，则不需要任何更改。这样每次上游更新，插件就会自动覆盖你的项目以达到同步上游的目的，缺点是你所有的更改都会被舍弃，完全跟上游一模一样。
3. 如果你想保留自己的更改，则只需将可以将.github目录下的pullyml文件名改为pull.yml。确保.github/pull.yml文件正常存在，yml内上游作者填写正确(此项目内容已填好，无需更改)。
3. ENJOY!上游更改三小时左右就会自动提起同步。

#### 注意

> 选择步骤3的情况下，除以下代码外都会自动同步 无需干涉
